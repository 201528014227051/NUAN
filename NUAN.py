...
class SimpleNet_simpler(nn.Module):
...
#         super(SimpleNet, self).__init__()
#         if output_activation == None:
#             self.fc = nn.Sequential(weight_norm(nn.Linear(input_dim,output_dim), dim=None),
#                                         nn.Dropout(dropout_rate)
#                                         )
#         else:
#             self.fc = nn.Sequential(weight_norm(nn.Linear(input_dim,output_dim), dim=None),
...
class MARN(nn.Module):
    def __init__(self,opt):
        ...
        self.vision_atten_dim = self.hidden_dims[0] + self.hidden_dims[1]  # vision + language
        self.acoustic_atten_dim = self.hidden_dims[0] + self.hidden_dims[2]  # vision + acoustic
...
#        self.batch_size = opt.batch_size
        ...
        vision_attention = SimpleNet(self.vision_atten_dim, self.attn_cell_dim*self.attn_num,
                              self.attn_dropout_rate,self.vision_atten_dim*self.attn_num,nn.Sigmoid())
                              
        vision_reduced = SimpleNet(self.vision_atten_dim*self.attn_num, self.attn_cell_dim*self.attn_num,
                              self.attn_dropout_rate,self.hidden_dims[1]*self.attn_num) # semantic vector
                              
        acoustic_attention = SimpleNet(self.acoustic_atten_dim, self.attn_cell_dim*self.attn_num,
                              self.attn_dropout_rate,self.acoustic_atten_dim*self.attn_num,nn.Sigmoid())
                              
        acoustic_reduced = SimpleNet(self.acoustic_atten_dim*self.attn_num, self.attn_cell_dim*self.attn_num,
                              self.attn_dropout_rate,self.hidden_dims[2]*self.attn_num)
        ...
        self.multi_attention = MultipleAttention(vision_attention, acoustic_attention, vision_reduced, acoustic_reduced, compression_nets, self.attn_num)
    ...
    def forward(self, in_modalities):
        ...
        total_output = torch.cat([z,*h],dim = 1) #*h取出列表中的所有的放入新的列表
...
class MultipleAttention(nn.Module):
    ...
    def __init__(self,vision_attention, acoustic_attention, vision_reduced, acoustic_reduced, dim_reduce_nets,num_atts):
        super(MultipleAttention, self).__init__()
        self.vision_attention = vision_attention
        self.acoustic_attention = acoustic_attention
        self.vision_reduced = vision_reduced
        self.acoustic_reduced = acoustic_reduced
...
#        input_dim = sum([modality.shape[1] for modality in in_modalities])
        ...
        # get the lan and vision
        vision_lan = torch.cat([in_modalities[0], in_modalities[1]], dim=1)
        vision_atten = self.vision_attention(vision_lan).view(batch_size,self.num_atts,-1)
        
                # get the lan and vision
        acoustic_lan = torch.cat([in_modalities[0], in_modalities[2]], dim=1)
        acoustic_atten = self.acoustic_attention(acoustic_lan).view(batch_size,self.num_atts,-1)
        
        #calculating attentions
        #atts=torch.softmax(attention,dim=2).view(batch_size,-1)
        vis_atts = torch.softmax(vision_atten,dim=2).view(batch_size, -1)
        acoustic_atts = torch.softmax(acoustic_atten,dim=2).view(batch_size, -1)
        ...
        vis_tensor=torch.cat([in_modalities[i].repeat(1,self.num_atts) for i in [0,1]],dim=1)
        acoustic_tensor=torch.cat([in_modalities[i].repeat(1,self.num_atts) for i in [0,2]],dim=1)
        
        #calculating the attention
        #att_out=atts*out_tensor
        vis_out = vis_atts*vis_tensor
        acoustic_out = acoustic_atts*acoustic_tensor
        
        vis_out = self.vision_reduced(vis_out)
        acoustic_out = self.acoustic_reduced(acoustic_out)
        ...
        out_modalities = [in_modalities[0].repeat(1,self.num_atts), vis_out, acoustic_out] # lan not changed
